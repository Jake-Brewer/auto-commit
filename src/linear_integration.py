...
def get_issue_comments(issue_id: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        from mcp_linear_oauth_list_comments import \
            mcp_linear_oauth_list_comments
...
def close_linear_issue(issue_id: str, team_id: str) -> bool:
    ...
    try:
        ...
        from mcp_linear_oauth_get_issue_status import \
            mcp_linear_oauth_get_issue_status

        done_status = mcp_linear_oauth_get_issue_status(query="Done", teamId=team_id)

        if done_status and "id" in done_status:
            # Update the issue to Done status
            return update_linear_issue(issue_id, {"stateId": done_status["id"]})
        else:

...
class LinearFallbackManager:
    ...
    def create_commit_message_request(self, diff_content: str, 
                                       file_paths: list) -> str:
        ...
        try:
            ...
            logger.info(f"Created Linear fallback issue {issue_id} for commit message request")
            return issue_id
    ...
    def poll_for_commit_message(self, issue_id: str) -> Optional[str]:
        ...
        # Timeout reached
        self._pending_issues.pop(issue_id, None)
        raise LinearFallbackError(f"Timeout waiting for response on Linear issue {issue_id}")
...
"""
    ...
    def _is_valid_commit_message(self, message: str) -> bool:
        ...
        if first_line.endswith('?') or first_line.lower().startswith(('please', 'can you', 'how')):
            return False

...
class TestConfigurationManager:
    ...
    def test_safe_add_default_ignores(self, mock_git_repo_class, config_manager, temp_dir):
    ...
    @patch('src.config_manager.GitRepo')
    def test_safe_add_default_ignores_with_conflicts(self, mock_git_repo_class, config_manager, temp_dir):
        """Test safe addition when tracked files would conflict."""
        # Mock GitRepo with tracked files that match some default patterns
        mock_repo = Mock()
        mock_repo.repo = Mock()
        mock_repo.get_tracked_files.return_value = ["build/config.py", "dist/package.tar.gz"]
        mock_git_repo_class.return_value = mock_repo
        
        # Mock the pattern matching to return True for conflicting patterns
        with patch.object(config_manager, '_has_tracked_files_matching_pattern') as mock_match:
            mock_match.side_effect = lambda repo, pattern: pattern in ["build/", "dist/"]
